# chatbot_exam_analyzer_final.py
# ระบบตรวจสอบมาตรฐานข้อสอบคณิตศาสตร์ ม.4 พร้อมวิเคราะห์ตาม Bloom's Taxonomy
# ปรับปรุงเพื่อ Deploy บน Streamlit Cloud ได้
...
# ✅ ส่วน OCR - ตรวจสอบความสามารถอย่างปลอดภัย
OCR_IMPORTED_SUCCESSFULLY = False
try:
    import pytesseract
    from pdf2image import convert_from_path
    OCR_IMPORTED_SUCCESSFULLY = True
    st.sidebar.success("✅ ระบบ OCR พร้อมใช้งาน")
except ImportError as e:
    OCR_IMPORTED_SUCCESSFULLY = False
    st.sidebar.warning("⚠️ ระบบ OCR ไม่พร้อมใช้งาน (ต้องติดตั้ง pytesseract และ pdf2image)")
    # แสดงข้อความ error แบบละเอียดขึ้นใน sidebar สำหรับผู้ดูแล
    # st.sidebar.text(f"รายละเอียด: {e}") # สามารถ uncomment ได้ถ้าต้องการ
...
def ocr_pdf_pages(file_path, dpi=300):
    ...
    # *******************************************************************************
    # ตรวจสอบ availability ของ OCR ก่อน
    # *******************************************************************************
    if not OCR_IMPORTED_SUCCESSFULLY:
        st.error("ระบบ OCR ไม่ได้ถูกติดตั้ง ไม่สามารถประมวลผล PDF สแกนได้")
        return ""

    try:
        # *******************************************************************************
        # ดึงค่า path จาก environment หรือใช้ default
        # *******************************************************************************
        poppler_path_from_env = os.getenv('POPPLER_PATH')
        
        # ถ้าไม่มี environment variable ให้ใช้ default ของระบบ
        # convert_from_path จะพยายามหา poppler library ใน system path
        
        images = convert_from_path(
            file_path,
            # poppler_path=POPPLER_PATH, # ไม่ใช้ค่าตายตัวจากโค้ด
            poppler_path=poppler_path_from_env, # ใช้ค่าจาก environment ถ้ามี
            dpi=dpi,
...
def validate_system_requirements():
    ...
    ปรับปรุงให้ปลอดภัยขึ้นบนแพลตฟอร์มที่ไม่มี Tesseract/Poppler
    """
    errors = []
    warnings = []

    # *******************************************************************************
    # ตรวจสอบ Tesseract - เฉพาะถ้าพยายามใช้ OCR
    # *******************************************************************************
    if OCR_IMPORTED_SUCCESSFULLY:
        # พยายาม validate paths โดยไม่ให้ crash ถ้า path ไม่ถูกต้อง
        try:
            # ดึงค่า default ของ tesseract_cmd
            default_tess_cmd = pytesseract.pytesseract.tesseract_cmd
            # ถ้า path ไม่ถูกต้อง แสดงเป็น warning แทน error
            if not os.path.isfile(default_tess_cmd):
                 # ถ้า path ไม่ถูกต้อง แสดงเป็น warning แทน error
                 warnings.append(f"Tesseract-OCR ไม่พบที่ path: {default_tess_cmd} (ระบบจะใช้ค่า default ถ้ามี)")
                 # ไม่ตั้ง pytesseract.pytesseract.tesseract_cmd ใหม่ เพื่อใช้ค่า default ของระบบ
                 
        except AttributeError:
            # กรณีที่ tesseract_cmd ไม่ได้ถูก define อย่างถูกต้อง
            warnings.append("ไม่สามารถดึงค่า default path ของ Tesseract-OCR ได้")
        except Exception as e:
            warnings.append(f"ไม่สามารถตรวจสอบ Tesseract ได้: {e}")

    # *******************************************************************************
    # ตรวจสอบ Poppler - เฉพาะถ้าพยายามใช้ OCR
    # *******************************************************************************
    if OCR_IMPORTED_SUCCESSFULLY:
        try:
            # สมมุติว่า POPPLER_PATH ถูก define ใน environment หรือที่อื่น
            # ถ้าไม่มี ให้ใช้ค่า default หรือ None
            poppler_path_from_env = os.getenv('POPPLER_PATH') # ดึงจาก environment variable
            if poppler_path_from_env:
                if not os.path.isdir(poppler_path_from_env):
                    warnings.append(f"Poppler ไม่พบที่ path จาก Environment: {poppler_path_from_env}")
            else:
                 # ถ้าไม่มี environment variable ให้ถือว่าใช้ default หรือ system path
                 # โดยไม่ต้องตรวจสอบ
                 pass # ไม่ต้องทำอะไร

        except Exception as e:
            warnings.append(f"ไม่สามารถตรวจสอบ Poppler ได้: {e}")

    # *******************************************************************************
    # แสดง warnings (ถ้ามี) - แสดงแค่ใน sidebar หรือ debug mode
    # *******************************************************************************
    # สำหรับการ deploy บน cloud ที่ไม่มี OCR ไม่จำเป็นต้องแสดง error
    # แต่สำหรับ local dev อาจมีประโยชน์
    # สามารถ uncomment ส่วนนี้เพื่อดู log ได้
    # if warnings:
    #     st.sidebar.info("ℹ️ ข้อมูลเพิ่มเติมจากการตรวจสอบระบบ:")
    #     for w in warnings:
    #         st.sidebar.markdown(f"- {w}")

    return errors # คืนเฉพาะ errors ที่สำคัญจริงๆ ที่จะทำให้ระบบใช้ไม่ได้
    ...
    if uploaded_file is None:
        ...
        with st.spinner("กำลังประมวลผลไฟล์ PDF..."):
            try:
                ...
                if not text.strip() and OCR_IMPORTED_SUCCESSFULLY:
...
- ระบบจะวิเคราะห์จำนวนข้อสอบและตัวเลือก
- ตรวจสอบว่าผ่านเกณฑ์มาตรฐานที่กำหนดหรือไม่
- วิเคราะห์ระดับความยากตาม Bloom's Taxonomy
""")
...
**รองรับ:**
- ไฟล์ PDF ปกติ
- ไฟล์ PDF สแกน (ถ้ามี Tesseract และ Poppler)
